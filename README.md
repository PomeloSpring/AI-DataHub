...
| **Database** | Apache Doris (analytics), MySQL/SQLite (metadata), Qdrant/Doris/In-Memory (vectors) |
...
│   │   ├── config.py           # Environment config (incl. LANGFUSE_*, QDRANT_*)
│   │   ├── db/                 # MetadataDB + VectorDB pools (MySQL/SQLite/Doris)
│   │   ├── llm/                # LLM client, embedding, token estimation, Langfuse client
│   │   ├── vector/             # VectorStore (Qdrant / Doris HNSW / In-Memory)
...
│   └── qdrant/                 # Qdrant vector DB deployment
│       └── docker-compose.yml  # Standalone Qdrant service
├── sync/                       # Database utilities
│   ├── metadata_sync.py        # Sync table metadata + embeddings to vector store
...
```bash
# Metadata Database (MySQL or SQLite)
METADATA_DB_TYPE=mysql              # mysql | sqlite | doris
METADATA_DB_HOST=127.0.0.1
METADATA_DB_PORT=3306
METADATA_DB_DATABASE=adh
# METADATA_DB_PATH=data/metadata.db  # SQLite only

# Vector Database (Qdrant / Doris / In-Memory)
VECTOR_DB_TYPE=qdrant               # qdrant | doris | default(in-memory)

# Qdrant (when VECTOR_DB_TYPE=qdrant)
QDRANT_HOST=127.0.0.1
QDRANT_PORT=6333
QDRANT_GRPC_PORT=6334
QDRANT_API_KEY=                     # optional
QDRANT_COLLECTION_PREFIX=           # optional, e.g. "prod_"

# Doris (when VECTOR_DB_TYPE=doris or METADATA_DB_TYPE=doris)
