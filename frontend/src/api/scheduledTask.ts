...
export interface ScheduledTask {
  ...
  trigger_type: 'cron' | 'webhook' | 'both';
...
export async function regenerateWebhookToken(id: number): Promise<{ success: boolean; webhook_token: string }> {
  const { data } = await client.post(`/scheduled-tasks/${id}/regenerate-webhook-token`);
  return data;
}
