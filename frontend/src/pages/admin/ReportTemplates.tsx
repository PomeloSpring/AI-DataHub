...
export default function ReportTemplates() {
  ...
  const handleCopy = (tpl: ReportTemplate) => {
    setEditTpl(null); // null = create mode
    setName(`${tpl.name} (副本)`);
    setDescription(tpl.description || '');
    setContent(tpl.content);
    setFormat(tpl.format);
          ...
          templates.map(tpl => (
            <div key={tpl.id} className="border rounded-lg p-4 space-y-3">
              ...
              <div className="flex justify-end gap-1">
                <Button variant="ghost" size="sm" onClick={() => handleCopy(tpl)} title="复制">
                  <Copy className="w-4 h-4" />
                </Button>
                <Button variant="ghost" size="sm" onClick={() => openEdit(tpl)} title="编辑">
                  <Edit className="w-4 h-4" />
                </Button>
                {!tpl.is_system && (
                  <Button variant="ghost" size="sm" onClick={() => setDeleteTarget(tpl)} title="删除">
