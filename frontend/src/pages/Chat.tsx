...
export default function Chat() {
  ...
  const [atCategory, setAtCategory] = useState<'all' | 'tools' | 'agents'>('all');
  ...
  const {
    ...
    sendMessage, cancelMessage, respondToAsk, cancelAsk, respondToCodeProposal, cancelCodeProposal, updateMessageFeedback, setViewMode, analyzeData, predictData, clear,
  ...
  // Load system tools and agents dynamically for @ mention
  useEffect(() => {
    if (isAgentMode) {
      // Load system tools from pipeline API
      client.get('/pipeline/tools').then(({ data }) => {
        setSystemTools((data || []).map((t: any) => ({
          name: t.name,
          displayName: t.name,
          serverName: '系统',
      ...
      // Load agents from admin API
      client.get('/admin/agents').then(({ data }) => {
        setAgents((data.items || data || []).filter((a: any) => a.is_active));
      }).catch(() => {});
    }
  }, [isAgentMode]);
  ...
  const mcpToolList = isAgentMode ? mcpServers.flatMap((s: any) => {
  ...
  const agentList = isAgentMode ? agents.map((a: any) => ({
    name: a.name,
  ...
  // Combine system tools + MCP tools + agents
  const allMcpTools = isAgentMode ? [...systemTools, ...mcpToolList, ...agentList] : [];

  // Filtered items for @ mention (support category filter)
  const filteredTools = atFilter
    ? allMcpTools.filter((t: any) => {
        // Category filter: @tools:xxx or @agents:xxx
        if (atCategory !== 'all' && t.category !== atCategory) return false;
        const filter = atFilter.replace(/^(tools|agents):/, '');
        if (!filter) return true;
        return t.name.toLowerCase().includes(filter.toLowerCase()) ||
          (t.displayName || '').toLowerCase().includes(filter.toLowerCase()) ||
          t.description.toLowerCase().includes(filter.toLowerCase());
      })
    : (atCategory === 'all' ? allMcpTools : allMcpTools.filter((t: any) => t.category === atCategory));

  // Parse @tool mentions from input — returns display names (what user sees)
  const parseToolDisplayNames = (text: string): string[] => {
    const matches = text.match(/@[\w_]+/g) || [];
    return matches.map(m => m.substring(1)).filter(name => {
      return allMcpTools.some((t: any) => t.name === name);
    });
  };

  // Parse @tool mentions — returns backend names (what gets sent to API)
  // Since tools are loaded dynamically, name = backend name
  const parseMcpTools = (text: string): string[] => {
    return parseToolDisplayNames(text);
  };
    ...
    if (atIndex >= 0) {
      ...
      if (atIndex === 0 || textBeforeCursor[atIndex - 1] === ' ') {
        if (!afterAt.includes(' ') && afterAt.length < 30) {
          // Detect category prefix: @tools: or @agents:
          if (afterAt.startsWith('tools:')) {
            setAtCategory('tools');
            setAtFilter(afterAt.substring(6));
          } else if (afterAt.startsWith('agents:')) {
            setAtCategory('agents');
            setAtFilter(afterAt.substring(7));
          } else {
            setAtCategory('all');
            setAtFilter(afterAt);
          }
  ...
  const handleSend = () => {
    ...
    // Strip @tool mentions from the display message (both MCP and system tools)
    const cleanMessage = input.replace(/@[\w_]+(\s|$)/g, '').trim();
              ...
              <div className={`max-w-[96%] min-w-0 overflow-hidden ${msg.role === 'user' ? 'max-w-[70%]' : ''}`}>
                ...
                {msg.role === 'assistant' && (
                  <div className="bg-muted rounded-2xl rounded-tl-sm px-4 py-3 overflow-hidden">
                    ...
                    {/* Agent propose_code interactive card */}
                    {msg.pendingCodeProposal && (
                      <div className="mt-3">
                        <CodeViewer
                          code={msg.pendingCodeProposal.code}
                          description={msg.pendingCodeProposal.description}
                          requirements={msg.pendingCodeProposal.requirements}
                          onExecute={() => respondToCodeProposal(msg.pendingCodeProposal!.request_id, '__EXECUTE__')}
                          onCancel={() => cancelCodeProposal(msg.pendingCodeProposal!.request_id)}
                        />
                      </div>
                    )}
                    ...
                    {msg.execution_results && msg.execution_results.length > 0 && (
                      <div className="mt-3 space-y-2">
                        {msg.execution_results.map((execResult, idx) => (
                          <ExecutionResultCard
                            key={idx}
                            result={execResult}
                            onRerun={(code, reqs) => {
                              // Re-execute by sending a new message with the code
                              sendMessage(`请执行以下代码：\n\`\`\`python\n${code}\n\`\`\``, ['propose_code']);
                            }}
                          />
                        ))}
        ...
        <div className="flex-shrink-0 p-4 border-t bg-background">
          <div className="relative">
            ...
            {isAgentMode && atMenuOpen && filteredTools.length > 0 && (
              <div className="absolute bottom-full left-0 right-0 mb-1 bg-popover border rounded-lg shadow-lg max-h-64 overflow-y-auto z-50">
                {/* Category tabs */}
                <div className="flex border-b text-xs">
                  <button
                    className={`flex-1 px-3 py-1.5 ${atCategory === 'all' ? 'bg-accent text-accent-foreground font-medium' : 'text-muted-foreground hover:bg-muted'}`}
                    onClick={() => { setAtCategory('all'); setAtMenuIndex(0); }}
                  >
                    全部
                  </button>
                  <button
                    className={`flex-1 px-3 py-1.5 ${atCategory === 'tools' ? 'bg-accent text-accent-foreground font-medium' : 'text-muted-foreground hover:bg-muted'}`}
                    onClick={() => { setAtCategory('tools'); setAtMenuIndex(0); }}
                  >
                    🔧 工具
                  </button>
                  <button
                    className={`flex-1 px-3 py-1.5 ${atCategory === 'agents' ? 'bg-accent text-accent-foreground font-medium' : 'text-muted-foreground hover:bg-muted'}`}
                    onClick={() => { setAtCategory('agents'); setAtMenuIndex(0); }}
                  >
                    🤖 Agent
                  </button>
                </div>
                ...
                <div className="px-2 py-1 text-[10px] text-muted-foreground border-b">
                  {atCategory === 'all' && '输入 @tools: 或 @agents: 筛选类别'}
                  {atCategory === 'tools' && '@tools: 系统工具 + MCP 工具'}
                  {atCategory === 'agents' && '@agents: 子 Agent'}
                </div>
                {/* Items */}
                {filteredTools.map((tool: any, i: number) => (
                  ...
                  >
                    ...
                    <span className={`ml-2 text-xs ${tool.category === 'agents' ? 'text-blue-500' : tool.isBuiltin ? 'text-orange-500' : 'text-muted-foreground'}`}>
                      ({tool.category === 'agents' ? 'Agent' : tool.serverName})
                    </span>
                ...
                {filteredTools.length === 0 && (
                  <div className="px-3 py-4 text-sm text-muted-foreground text-center">
                    未找到匹配的{atCategory === 'agents' ? 'Agent' : '工具'}
                  </div>
                )}
              </div>
            )}
            ...
            <div className="flex gap-2">
              <Input
                ...
                placeholder={isAgentMode ? "输入问题... @tools:工具 @agents:Agent" : "输入你的数据查询问题..."}
                className="flex-1"
            ...
            {isAgentMode && parseToolDisplayNames(input).length > 0 && (
              <div className="flex items-center gap-1 mt-1.5 flex-wrap">
                <span className="text-xs text-muted-foreground">调用:</span>
                {parseToolDisplayNames(input).map(toolName => (
