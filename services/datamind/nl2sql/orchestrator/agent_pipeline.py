...
from services.datamind.agent.base import AgentResult
...
# System tools the orchestrator can call directly (for simple queries & fallback)
...
def _estimate_tokens(text: str) -> int:
    ...
    from services.shared.common.llm.token_estimator import estimate_tokens
    return estimate_tokens(text)


def _estimate_messages_tokens(messages: list[dict]) -> int:
    """Estimate total tokens in a message list."""
    from services.shared.common.llm.token_estimator import estimate_messages_tokens
...
ORCHESTRATOR_TOOLS = [
    ...
    {
        "name": "ask_user",
        "description": "当你无法确定应该调用哪个子 Agent、或需要用户澄清问题时，调用此工具向用户提问。提供选项让用户点击选择。",
        "input_schema": {
            "type": "object",
            "properties": {
                "question": {
    ...
    # === Multimodal / OpenCV 图像工具 ===
    {
        "name": "image_info",
        "description": "【多模态】获取用户上传图片附件的基础信息(尺寸/通道/亮度/对比度)。需要用户提供 attachment_id。",
        "input_schema": {
            "type": "object",
            "properties": {
                "attachment_id": {
                    "type": "string",
                    "description": "图片附件 ID(来自用户上传的附件信息)",
                },
            },
    ...
    {
        "name": "image_process",
        "description": "【多模态】对用户上传的图片附件做 OpenCV 处理:resize(缩放)/crop(裁剪)/grayscale(灰度)/edges(边缘检测)。产物另存为新附件并返回访问 URL。",
        "input_schema": {
            "type": "object",
            "properties": {
                "attachment_id": {"type": "string", "description": "图片附件 ID"},
                "operation": {
                    "type": "string",
                    "enum": ["resize", "crop", "grayscale", "edges"],
                    "description": "处理操作",
                },
                "params": {
                    "type": "object",
                    "description": "操作参数:resize 需 {width};crop 需 {x,y,width,height};其余可省略",
                },
            },
    ...
    {
        "name": "detect_table_region",
        "description": "【多模态】检测用户上传图片中的表格区域(最大四边形轮廓)并裁剪保存,用于扫描表格/图表截图前处理。",
        "input_schema": {
            "type": "object",
            "properties": {
                "attachment_id": {"type": "string", "description": "图片附件 ID"},
...
async def _execute_system_tool(
    ...
    try:
        ...
        if tool_name == "list_tables":
            ...
            import pymysql
            from services.shared.common.config import DORIS_HOST, DORIS_PORT, DORIS_USER, DORIS_PASSWORD, METADATA_DB_DATABASE
        ...
        # ── SQL Generation & Execution ─────────────────────────────
            ...
            # ── Detect system/metadata queries (should skip DataEngine) ──
            # DataFusion only has business tables; information_schema / SHOW / DESCRIBE
            # must go directly to the underlying database.
            _sql_up = sql.strip().upper()
            _is_system_query = (
                _sql_up.startswith("SHOW ")
                or _sql_up.startswith("DESCRIBE ")
                or _sql_up.startswith("EXPLAIN ")
                or "INFORMATION_SCHEMA" in _sql_up
                or "information_schema" in sql
            )

            if _is_system_query and query_type == "sql":
                # Skip DataEngine and verification — go straight to the database
                import time as _t
                import pandas as _pd
                from services.datamind.nl2sql.sql.query_executor import get_connection
                _t0 = _t.time()
                try:
                    with get_connection(datasource_id) as _conn:
                        with _conn.cursor() as _cur:
                            _cur.execute(sql)
                            _rows = _cur.fetchall()
                            df = _pd.DataFrame(_rows) if _rows else _pd.DataFrame()
                    _elapsed = int((_t.time() - _t0) * 1000)
                    columns = list(df.columns) if not df.empty else []
                    rows = [_sanitize_row(r) for r in df.head(100).to_dict(orient="records")] if not df.empty else []
                    return json.dumps({
                        "columns": columns,
                        "rows": rows,
                        "row_count": len(df),
            ...
            if not sql and query_type == "sql":
                return json.dumps({
                    "error": "未提取到有效的 SQL 语句。请确保返回的是可执行的 SELECT 查询，而不是解释说明。",
                    "hint": "请直接生成 SQL 语句，不要在 SQL 中包含中文解释。",
                }, ensure_ascii=False)

            # Try engine server first (MDL + RLS + dialect transpile)
            from services.shared.common.engine_client import ENGINE_ENABLED
            user_context = {"user_id": user_id, "username": username, "role": user_role}
            ...
            if ENGINE_ENABLED and query_type == "sql":
                ...
                try:
                    ...
                    logger.warning("[Agent] Engine execution failed, falling back: %s", engine_err)
                    # Fallback: use lightweight permission enforcement
                    from services.datamind.nl2sql.sql.query_executor import execute_query_with_permission
                    try:
                        df, elapsed_ms, row_count = execute_query_with_permission(
                            sql=sql,
                ...
                # No engine: use lightweight permission enforcement
                from services.datamind.nl2sql.sql.query_executor import execute_query_with_permission
                try:
                    df, elapsed_ms, row_count = execute_query_with_permission(
                        sql=sql,
        ...
        # ── Analysis Skill Loading ──────────────────────────────────
            ...
            if not skill_name:
                return json.dumps({"error": "必须提供 skill_name"})
            skill = load_skill(skill_name)
            if not skill:
                return json.dumps({"error": f"未找到技能: {skill_name}"})
            return json.dumps({
        ...
        # ── Multimodal / OpenCV Image Tools ─────────────────────

        elif tool_name in ("image_info", "image_process", "detect_table_region"):
            from services.datamind.multimodal.loader import load_attachments
            from services.datamind.multimodal import opencv_tools

            att_id = tool_input.get("attachment_id", "")
            if not att_id:
                return json.dumps({"error": "必须提供 attachment_id 参数"}, ensure_ascii=False)
            atts = load_attachments([att_id], user_id)
            if not atts:
                return json.dumps({"error": f"附件不存在或无权访问: {att_id}"}, ensure_ascii=False)
            att = atts[0]
            if att.get("category") != "image":
                return json.dumps({"error": f"附件 {att_id} 不是图片({att.get('category')})"}, ensure_ascii=False)

            if tool_name == "image_info":
                result = opencv_tools.image_info(att)
            elif tool_name == "image_process":
                result = opencv_tools.image_process(att, tool_input.get("operation", ""), tool_input.get("params") or {})
            else:
...
async def _try_fast_path(
    ...
    user_id: int,
    username: str,
    allowed_datasource_ids: list,
    engine: str,
) -> dict | None:
    """Try to handle simple metadata/SQL queries without LLM loop.

    Returns dict with {reply, sql?, data?, elapsed_ms?} if fast path applies,
    or None if the question needs full agent processing.

    Fast path handles:
    - "有几个表" / "有哪些表" / "表列表" → list_tables or direct SQL
    - Simple count queries that map to metadata
    """
    import re
    q = question.strip()

    # Pattern: "有几个表" / "有哪些表" / "多少张表" / "表的数量"
    table_count_patterns = [
        r"有几[个张]表", r"多少[个张]表", r"表的[数量]",
        r"表列表", r"所有表", r"全部表",
        r"[listshow].*table", r"how many table",
    ]
    is_table_query = any(re.search(p, q, re.IGNORECASE) for p in table_count_patterns)
    ...
    try:
        ...
        if isinstance(data, dict) and data.get("success"):
            tables = data.get("tables", [])
            table_names = [t.get("table_name", t.get("name", str(t))) for t in tables]
            reply = f"当前数据源中共有 **{len(table_names)}** 张表：\n\n"
            reply += "\n".join(f"- {name}" for name in table_names)
            elapsed = round((time.time() - t_start) * 1000)
            return {"reply": reply, "elapsed_ms": elapsed}
    except Exception as e:
        logger.warning("[FastPath] list_tables failed: %s, falling back to agent", e)
...
async def agent_generate(
    ...
    Args:
        ...
        attachments: 多模态附件 ID 列表;构建用户消息时转为 content blocks(图片/表格/文档/3D).
    """
    from services.shared.common.llm.llm_client import _get_model_config, generate_with_tools_stream
    ...
    if workspace_id:
        from services.dataflow.services.workspace_service import get_workspace_service
    ...
    # 2. Collect tools — orchestrator tools + SYSTEM_TOOLS + MCP tools + agent tools
    # Orchestrator has direct access to SYSTEM tools for:
    #   - Simple queries (metadata, quick SQL) without sub-agent overhead
    #   - Fallback when sub-agents fail
    tools = list(ORCHESTRATOR_TOOLS)
    # Add system tools so orchestrator can handle simple queries directly
    for t in SYSTEM_TOOLS:
        if t["name"] in ORCHESTRATOR_SYSTEM_TOOL_NAMES:
            tools.append(t)
    if disable_ask_user:
    ...
    # Include system tools that orchestrator can directly call
    for t in SYSTEM_TOOLS:
        if t["name"] in ORCHESTRATOR_SYSTEM_TOOL_NAMES:
            tool_descs.append(f"- {t['name']}: {t['description']}")
    ...
    from services.datamind.config.agent_loader import build_agent_graph
...
"""
    ...
    # 6. Build initial messages(支持多模态附件)
    ...
    if attachments:
        from services.datamind.multimodal.loader import load_attachments, build_user_content
        att_rows = load_attachments(attachments, user_id or 0)
        model_config = _get_model_config(model_id)
        supports_vision = bool(model_config.get("supports_vision", 1))
        messages.append({"role": "user", "content": build_user_content(question, att_rows, supports_vision)})
        att_desc = ", ".join(f"{a['filename']}({a['category']}, id={a['id']})" for a in att_rows)
        system_prompt += (
            f"\n\n## 多模态附件\n用户本次上传了 {len(att_rows)} 个附件: {att_desc}。"
            "图片可用 image_info / image_process / detect_table_region 工具做 OpenCV 分析处理。"
        )
    ...
    # ── Fast Path: simple metadata queries bypass LLM loop ──────────
    fast_result = await _try_fast_path(
        question, datasource_id, model_id, user_id, username,
        allowed_datasource_ids, engine,
    )
    if fast_result is not None:
        yield "progress", {"stage": "fast_path", "message": "快速查询", "mode": "fast"}
        yield "token", {"text": fast_result["reply"]}
        yield "done", {
            "input": 0, "output": 0,
            "sql": fast_result.get("sql", ""),
            "result": fast_result.get("data"),
            "elapsed_ms": fast_result.get("elapsed_ms", 0),
        }
        return
                ...
                try:
                    ...
                    if recovered_tables:
                        ...
                        if parsed.get("success") and parsed.get("sql"):
                            ...
                            try:
                                from services.datamind.nl2sql.sql.sql_validator import validate_and_fix
                                from services.datamind.nl2sql.sql.query_executor import execute_query as _exec_q
                                ...
                                if final_sql:
                                    ...
                                    logger.warning("[Agent] Auto-recovery produced empty SQL")
