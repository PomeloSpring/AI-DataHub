        ...
        try:
            ...
            try:
                with conn.cursor() as cur:
                    # Parse tags
                    import json
                    tags_json = json.dumps(document.get("tags", [])) if document.get("tags") else None
                    ...
                    """, (
                        ...
                        document.get("doc_type", "custom_text"),
        ...
        try:
            ...
            try:
                with conn.cursor() as cur:
                    # 获取文档
                    cur.execute(
                        "SELECT id, title, content, doc_type FROM adh_knowledge_documents WHERE id = %s",
                        (doc_id,)
                    )
                    doc = cur.fetchone()
                    if not doc:
                    ...
                    # 获取文档分块
                    cur.execute(
                        "SELECT id, content FROM adh_knowledge_chunks WHERE document_id = %s",
                        (doc_id,)
                    )
                    chunks = cur.fetchall()
                    ...
                    if not chunks:
                        ...
                        chunks_text = self._chunk_document(content)
                        for i, chunk_text in enumerate(chunks_text):
                            chunk_id = f"{doc_id}_chunk_{i}"
                            cur.execute(
                                """INSERT INTO adh_knowledge_chunks (id, document_id, content, chunk_index)
                                   VALUES (%s, %s, %s, %s)""",
                    ...
                    # 生成向量嵌入
                    from services.shared.common.llm.embedding import get_embedding
                    import json
                    ...
                    for chunk in chunks:
                        ...
                        cur.execute(
                            "UPDATE adh_knowledge_chunks SET embedding = %s WHERE id = %s",
                            (embedding_json, chunk["id"])
                        )

                    # 更新文档状态
                    cur.execute(
                        "UPDATE adh_knowledge_documents SET has_embedding = 1 WHERE id = %s",
                        (doc_id,)
                    )
                    ...
                    return {
                        ...
                        "message": f"Vectorized {len(chunks)} chunks"
                    }
            finally:
                conn.close()
