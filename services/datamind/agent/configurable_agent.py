...
class ConfigurableAgent(BaseAgent):
    ...
    def __init__(self, db_config: dict):
        ...
        from services.datamind.config.agent_loader import load_agent_skill, load_agent_prompt
        ...
        from services.datamind.config.agent_loader import get_max_iterations, get_max_retries
    ...
    async def _collect_tools(self) -> list[dict]:
        ...
        # Inject built-in tools (sandbox code execution + file toolbox)
        ...
        return self._apply_tool_permission(tools)

    def _apply_tool_permission(self, tools: list[dict]) -> list[dict]:
        """按工作空间绑定的 allowed_tools 白名单过滤工具(空=不限制).

        白名单支持标准工具名(read/write/bash...)与具体工具名
        (如 mcp__srv__tool),标准名按 builtin 映射展开。
        """
        if not self._allowed_tools:
            return tools
        from services.datamind.execution.tool_catalog import expand_allowed_tools

        allowed = expand_allowed_tools(self._allowed_tools, "builtin")
        filtered = [t for t in tools if t["name"] in allowed]
        logger.info("[ConfigAgent:%s] Tool permission: allowed=%s -> %s",
                    self.name, self._allowed_tools, [t["name"] for t in filtered])
        return filtered

    def _get_builtin_tools(self) -> list[dict]:
        """Get built-in tools (sandbox code execution + builtin toolbox)."""
        tools = []

        # builtin toolbox — read/write/edit/glob/grep/bash/webfetch,在工作空间沙箱目录内
        if self.extra_config.get("enable_file_tools", True):
            from services.datamind.agent.file_tools import BuiltinToolbox
            tools.extend(BuiltinToolbox.tool_definitions())
...
"""
    ...
    async def _execute_run_code(self, arguments: dict) -> str:
        ...
        try:
            from services.aiplatform.services.sandbox_service import sandbox_service
            from services.aiplatform.services.sandbox_executor import SandboxExecutor
        except ImportError:
            return "错误: 沙箱服务未部署，请联系管理员"
