...
async def get_graph_visualization(
    ...
    try:
        from services.shared.graphservice.graph_service import GraphService
    ...
    try:
        ...
        for r in results:
            nodes.append({
                ...
                "type": r.get("type", {}).get("value", "").replace(ADH_NS, "") if r.get("type") else (node_type or "Unknown"),
...
async def get_neighbors(node_iri: str, depth: int = Query(1, ge=1, le=3),
    ...
    try:
        ...
        for r in results:
            neighbors.append({
                "iri": r.get("iri", {}).get("value", ""),
                "type": r.get("type", {}).get("value", "").replace(ADH_NS, ""),
                "label": r.get("label", {}).get("value", ""),
                "comment": r.get("comment", {}).get("value", "") if r.get("comment") else "",
            })
...
async def graph_stats(datasource_id: int = Query(0, ge=0)):
    ...
    try:
        ...
        for r in type_counts:
            type_val = r.get("type", {}).get("value", "").replace(ADH_NS, "")
