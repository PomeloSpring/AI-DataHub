...
Usage:
    ...
    celery -A services.dataflow.tasks.celery_app worker -Q scheduled,default -l info -c 4

    # Start Beat (single instance, Redis lock prevents duplicates):
    celery -A services.dataflow.tasks.celery_app beat -l info

    # Optional monitoring:
    celery -A services.dataflow.tasks.celery_app flower
"""
...
app.autodiscover_tasks(["services.dataflow.tasks"])
