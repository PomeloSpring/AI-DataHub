...
class ScheduledTaskService:
    ...
    def create_task(self, data: dict, owner_id: int, workspace_id: int = 0) -> int:
        ...
        webhook_token = secrets.token_urlsafe(32) if trigger_type in ("webhook", "both") else None
        conn = get_metadata_conn()
        try:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    "cron_expression, trigger_type, webhook_token, webhook_secret, "
                    "timezone, channel_id, notify_on_success, notify_on_failure, "
    ...
    def update_task(self, task_id: int, data: dict) -> bool:
        ...
        try:
            with conn.cursor() as cur:
                ...
                # Auto-generate webhook_token when trigger_type changes to include webhook
                if "trigger_type" in data and data["trigger_type"] in ("webhook", "both"):
                    cur.execute("SELECT webhook_token FROM adh_scheduled_tasks WHERE id = %s", (task_id,))
                    existing = cur.fetchone()
                    if existing and not existing.get("webhook_token"):
                        updates.append("webhook_token = %s")
                        params.append(secrets.token_urlsafe(32))
    ...
    def get_task_by_webhook_token(self, task_id: int, token: str) -> Optional[dict]:
        """Get a task by ID and webhook token for webhook trigger authentication."""
        conn = get_metadata_conn()
        try:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT * FROM adh_scheduled_tasks WHERE id = %s AND webhook_token = %s AND is_active = 1",
                    (task_id, token),
                )
                row = cur.fetchone()
                if row:
    ...
    def regenerate_webhook_token(self, task_id: int) -> Optional[str]:
        """Regenerate webhook token for a task. Returns the new token."""
        new_token = secrets.token_urlsafe(32)
        conn = get_metadata_conn()
        try:
            with conn.cursor() as cur:
                cur.execute(
                    "UPDATE adh_scheduled_tasks SET webhook_token = %s, updated_at = %s WHERE id = %s",
                    (new_token, _now(), task_id),
                )
            conn.commit()
