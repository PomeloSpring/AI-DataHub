...
请告诉我这些信息，我会帮你配置。"""
    ...
    async def upload_document(
        ...
        try:
            ...
            # 保存文档
            self._save_document(doc_id, title, content, doc_type, tags, user_id)
    ...
    async def _retrieve_knowledge(
        ...
        try:
            ...
            # 尝试使用GraphRAG增强检索（仅在需要时）
            if len(sources) < 3:
                try:
                    from backend.rag.graph_rag import GraphRetriever
                    graph_retriever = GraphRetriever()

                    # 获取图上下文
                    graph_context = await graph_retriever.get_context_for_query(query)

                    # 添加相关表信息
                    if graph_context.get("direct_tables"):
                        for table in graph_context["direct_tables"][:2]:
                            sources.append(SourceInfo(
                                id=f"table:{table}",
                                title=f"表: {table}",
                                content=f"相关表: {table}",
                    ...
                    logger.debug(f"GraphRAG enhancement failed (non-critical): {e}")
...
请根据用户的问题提供准确的回答。如果用户问的是项目整体功能，介绍系统的核心能力。如果用户问的是具体功能，提供详细说明。"""
    ...
    async def _call_llm_with_tools(
        ...
        """
        try:
            from backend.common.llm.llm_client import generate_with_tools
