...
1. **理解问题**：分析用户到底在问什么，需要哪些数据
2. **查找表**：用 select_tables 找到相关表
3. **获取元数据**：用 retrieve_metadata 获取表结构、关联关系、业务术语
4. **生成 SQL**：用 generate_sql 生成 SQL（或自己写 SQL）
5. **校验 SQL**：用 validate_sql 校验 SQL 语法和安全性
6. **执行 SQL**：用 execute_sql 执行查询
7. **分析结果**：对查询结果进行分析和解读
...
"""
    ...
    def _get_system_tools(self) -> list[dict]:
        ...
        from backend.nl2sql.orchestrator.agent_constants import SYSTEM_TOOLS
    ...
    def _build_system_prompt(self, datasource_id: int = 0) -> str:
        ...
        from backend.config.skill_loader import get_skill_summary_for_prompt
...
"""
    ...
    async def _execute_system_tool(
        ...
        from backend.nl2sql.orchestrator.agent_pipeline import _execute_system_tool
