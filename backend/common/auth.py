"""
Authentication module — bcrypt password verification against Doris adh_users.
Enhanced with: password policy, login lockout, audit logging, user profile management.
        ...
        with _get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    "(id, user_id, username, action, module, target_type, target_id, detail, ip_address, created_at) "
                    "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                    (_ts_id(), user_id, username, action, module, target_type, target_id,
                     detail, ip_address, _now_str()),
                )
            conn.commit()
    except Exception as e:
    ...
    try:
        with _get_connection() as conn:
            with conn.cursor() as cur:
                ...
                offset = (page - 1) * size
                cur.execute(
                    f"SELECT id, user_id, username, action, module, target_type, target_id, "
