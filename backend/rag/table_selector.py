...
from backend.rag.terminology_manager import expand_synonyms
from backend.rag.bm25 import BM25, rrf_merge
...
def _get_all_tables(datasource_id: int = 0) -> list[dict]:
    ...
    from backend.rag.metadata_cache import get_cached_tables, set_cached_tables

    # Try distributed cache first
    cached = get_cached_tables(datasource_id)
    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                ...
                rows = cur.fetchall()
                _tables_cache[datasource_id] = rows
                set_cached_tables(datasource_id, rows)
                logger.info("Loaded %d active tables from adh_table_info (ds=%d)", len(rows), datasource_id)
        finally:
            conn.close()
...
def clear_cache(datasource_id: int = None):
    ...
    from backend.rag.metadata_cache import invalidate_datasource
    global _tables_cache, _bm25_cache
    if datasource_id is not None:
        _tables_cache.pop(datasource_id, None)
        _bm25_cache.pop(datasource_id, None)
        invalidate_datasource(datasource_id)
    else:
        _tables_cache.clear()
        _bm25_cache.clear()
        # Can't invalidate all Redis keys easily, rely on TTL
