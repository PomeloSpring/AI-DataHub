...
def create_llm_model(req: LLMModelCreate, admin: UserInfo = Depends(require_admin)):
    ...
    try:
        ...
        log_audit(admin.id, admin.username, "create_llm_model",
                  target_type="model", target_id=row_id,
                  detail=f"创建LLM模型 {req.name}", module="model")
...
def update_llm_model(model_id: int, req: LLMModelUpdate, admin: UserInfo = Depends(require_admin)):
    ...
    try:
        ...
        conn.commit()
        _invalidate_models_cache()
        log_audit(admin.id, admin.username, "update_llm_model",
                  target_type="model", target_id=model_id,
                  detail=f"更新LLM模型 {fields['name']}", module="model")
        return {"success": True}
    finally:
        conn.close()
...
def delete_llm_model(model_id: int, admin: UserInfo = Depends(require_admin)):
    ...
    try:
        ...
        conn.commit()
        _invalidate_models_cache()
        log_audit(admin.id, admin.username, "delete_llm_model",
                  target_type="model", target_id=model_id,
                  detail=f"删除LLM模型 id={model_id}", module="model")
        return {"success": True}
    finally:
        conn.close()
...
def set_default_model(model_id: int, admin: UserInfo = Depends(require_admin)):
    ...
    try:
        ...
        conn.commit()
        _invalidate_models_cache()
        log_audit(admin.id, admin.username, "set_default_model",
                  target_type="model", target_id=model_id,
                  detail=f"设置默认模型 id={model_id}", module="model")
        return {"success": True}
    finally:
        conn.close()
...
def update_embedding_config(req: EmbeddingConfigUpdate, admin: UserInfo = Depends(require_admin)):
    ...
    try:
        ...
        conn.commit()
        log_audit(admin.id, admin.username, "update_embedding_config",
                  target_type="model", detail="更新Embedding配置", module="model")
        return {"success": True}
    finally:
        conn.close()
...
def reload_embedding(admin: UserInfo = Depends(require_admin)):
    ...
    log_audit(admin.id, admin.username, "reload_embedding",
              target_type="model", detail=f"重载Embedding模型 {model_path}", module="model")
...
def update_system_config(req: SystemConfigUpdate, admin: UserInfo = Depends(require_admin)):
    ...
    try:
        ...
        conn.commit()
        log_audit(admin.id, admin.username, "update_system_config",
                  target_type="system", detail=f"更新系统配置 {req.key}", module="system")
        return {"success": True}
    finally:
        conn.close()
