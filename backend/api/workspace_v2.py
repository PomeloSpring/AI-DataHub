...
async def create_workspace(req: WorkspaceCreate, user: UserInfo = Depends(get_current_user)):
    ...
    service = get_workspace_service()
    try:
        result = await service.create_workspace(user.id, req.model_dump())
        log_audit(user.id, user.username, "create_workspace",
                  target_type="workspace", target_id=result.get("id", 0),
                  detail=f"创建工作空间 {req.name}", module="workspace")
        return result
...
async def update_workspace(workspace_id: int, req: WorkspaceUpdate, user: UserInfo = Depends(get_current_user)):
    ...
    try:
        result = await service.update_workspace(workspace_id, req.model_dump(exclude_unset=True))
        log_audit(user.id, user.username, "update_workspace",
                  target_type="workspace", target_id=workspace_id,
                  detail=f"更新工作空间 id={workspace_id}", module="workspace")
        return result
    except Exception as e:
...
async def delete_workspace(workspace_id: int, user: UserInfo = Depends(get_current_user)):
    ...
    try:
        ...
        log_audit(user.id, user.username, "delete_workspace",
                  target_type="workspace", target_id=workspace_id,
                  detail=f"删除工作空间 id={workspace_id}", module="workspace")
