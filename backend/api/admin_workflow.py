...
def create_prompt(req: PromptCreate, admin: UserInfo = Depends(require_admin)):
    ...
    try:
        ...
        conn.commit()
        log_audit(admin.id, admin.username, "create_prompt",
                  target_type="prompt", target_id=prompt_id,
                  detail=f"创建Prompt {req.prompt_key}", module="system")
...
def update_prompt(prompt_key: str, req: PromptUpdate, admin: UserInfo = Depends(require_admin)):
    ...
    try:
        ...
        conn.commit()
        log_audit(admin.id, admin.username, "update_prompt",
                  target_type="prompt", target_id=new_prompt_id,
                  detail=f"更新Prompt {prompt_key} v{new_version}", module="system")
...
def create_workflow(req: WorkflowConfigCreate, admin: UserInfo = Depends(require_admin)):
    ...
    try:
        ...
        conn.commit()
        log_audit(admin.id, admin.username, "create_workflow",
                  target_type="workflow", target_id=workflow_id,
                  detail=f"创建工作流 {req.name}", module="system")
...
def update_workflow(workflow_id: int, req: WorkflowConfigUpdate, admin: UserInfo = Depends(require_admin)):
    ...
    try:
        ...
        conn.commit()
        log_audit(admin.id, admin.username, "update_workflow",
                  target_type="workflow", target_id=workflow_id,
                  detail=f"更新工作流 id={workflow_id}", module="system")
        return {"success": True}
    finally:
        conn.close()
...
def delete_workflow(workflow_id: int, admin: UserInfo = Depends(require_admin)):
    ...
    try:
        ...
        conn.commit()
        log_audit(admin.id, admin.username, "delete_workflow",
                  target_type="workflow", target_id=workflow_id,
                  detail=f"删除工作流 id={workflow_id}", module="system")
        return {"success": True}
    finally:
        conn.close()
