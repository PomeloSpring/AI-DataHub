...
from backend.common.llm.embedding import (
    generate_embedding, get_model_info, reload_model,
...
def vector_search(req: SearchRequest, user: UserInfo = Depends(get_current_user)):
    ...
    if req.datasource_id:
        filters["datasource_id"] = req.datasource_id
    output_columns = ["table_name", "table_comment", "table_business_desc", "keywords"]
    steps.append({
    ...
    # Step 3: Execute vector search via VectorStore abstraction
    t0 = time.time()
    try:
    ...
    for i, row in enumerate(rows):
        results.append({
            ...
            "distance": round(row.get("distance", 0), 6),
        })
    steps.append({
        "name": "format_results",
        "duration_ms": round((time.time() - t0) * 1000, 2),
...
def column_vector_search(req: SearchRequest, user: UserInfo = Depends(get_current_user)):
    ...
    t0 = time.time()
    table_filters = {"is_active": 1}
    if req.datasource_id:
        table_filters["datasource_id"] = req.datasource_id
    try:
        store = get_vector_store()
        table_rows = store.search(
            table="adh_table_info",
            query_embedding=vec,
            limit=5,
            filters=table_filters,
            output_columns=["table_name"],
        )
    except Exception as e:
    ...
    if not table_names:
        ...
        col_filters = {"is_active": 1, "table_name": table_names}
        if req.datasource_id:
            col_filters["datasource_id"] = req.datasource_id
        try:
            rows = store.search(
                ...
                limit=req.limit,
                filters=col_filters,
                output_columns=["table_name", "column_name", "data_type", "column_comment", "business_desc"],
            )
        except Exception as e:
