...
def _execute_sql_on_datasource(sql: str, datasource_id: int) -> dict:
    ...
    from backend.api.dashboard import _execute_on_datasource
...
def _generate_report(task: dict, results: list) -> tuple[str, str]:
    ...
    try:
        ...
        template_path = Path(__file__).resolve().parent.parent / "config" / "templates" / f"{template_id}.md"
    ...
    from backend.config.agent_loader import load_agent_prompt
...
@app.task(
    name="backend.tasks.executor.execute_scheduled_task",
    bind=True,
    queue="scheduled",
    acks_late=True,
