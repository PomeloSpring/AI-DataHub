...
from backend.nl2sql.orchestrator.agent_constants import TOOL_RESULT_MAX_CHARS, DEFAULT_TOOL_RESULT_MAX
...
async def execute_system_tool(
    ...
    try:
        ...
        # ── Self-Correction ────────────────────────────────────────
            ...
            if table_names:
                try:
                    from backend.rag.rag_retriever import _get_table_info_for_names, _get_columns_for_tables
                    table_info = _get_table_info_for_names(table_names, datasource_id)
                    columns = _get_columns_for_tables(table_names, datasource_id)
                    if columns:
                        col_text = "\n".join(
