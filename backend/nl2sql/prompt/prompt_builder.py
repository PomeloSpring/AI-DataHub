...
from backend.nl2sql.sql.template_loader import get_sql_prompt, get_correction_prompt
...
def _to_knowledge_instructions(knowledge_items: list[dict]) -> str:
    """Format instruction-type knowledge items as prompt directives."""
    instructions = [k for k in knowledge_items if k.get("knowledge_type") == "instruction"]
    if not instructions:
        return ""

    lines = ["<Knowledge-Instructions>",
             "  <instruction>以下指令来自知识库，必须严格遵守：</instruction>"]
    for item in instructions:
        content = item.get("content", "").strip()
        if content:
            lines.append(f"  <rule>{content}</rule>")
    lines.append("</Knowledge-Instructions>")
    return "\n".join(lines)


def _to_knowledge_sql_pairs(knowledge_items: list[dict]) -> str:
    """Format sql_pair-type knowledge items as few-shot examples."""
    sql_pairs = [k for k in knowledge_items if k.get("knowledge_type") == "sql_pair"]
    if not sql_pairs:
        return ""

    lines = ["<Knowledge-SQL-Pairs>",
             "  <instruction>以下是从知识库中检索的问答对，可作为参考：</instruction>"]
    for item in sql_pairs:
        ...
        if isinstance(meta, str):
            ...
            try:
                ...
                meta = {}
        question = meta.get("question", item.get("title", ""))
        sql = meta.get("answer_sql", item.get("content", ""))
        explanation = meta.get("explanation", "")
        if question and sql:
            lines.append(f"  <example>")
            lines.append(f"    <question>{question}</question>")
            lines.append(f"    <sql>{sql}</sql>")
            if explanation:
                lines.append(f"    <explanation>{explanation}</explanation>")
            lines.append(f"  </example>")
    lines.append("</Knowledge-SQL-Pairs>")
    return "\n".join(lines)
...
def build_nl2sql_prompt(
    ...
    instructions_text = _to_knowledge_instructions(knowledge_items or [])
    knowledge_sql_pairs = _to_knowledge_sql_pairs(knowledge_items or [])

    # Compose system message: system rules + context
    context_parts = [tpl["system"]]

    # Add instructions from knowledge base (high priority)
    ...
    # Add SQL examples (from templates + knowledge sql_pairs)
    if examples_text or knowledge_sql_pairs:
        combined_examples = examples_text
        if knowledge_sql_pairs:
            if combined_examples:
                combined_examples += "\n\n" + knowledge_sql_pairs
            else:
                combined_examples = knowledge_sql_pairs
        context_parts.append(combined_examples)
