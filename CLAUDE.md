...
- **Database**: Apache Doris (analytical queries), MySQL/SQLite (metadata), Qdrant/Doris/In-Memory (vectors)
...
- **Metadata DB** (MySQL/SQLite/Doris): table/column/term metadata, user data, config
- **Vector DB** (Qdrant/Doris/In-Memory): vector embeddings for RAG retrieval
  - `VECTOR_DB_TYPE=qdrant` — Qdrant (recommended for production)
  - `VECTOR_DB_TYPE=doris` — Doris HNSW index
  - `VECTOR_DB_TYPE=default` — In-memory (numpy L2, dev only)
- Connections: `get_metadata_conn()` / `get_vector_conn()` — separate pools
- Vector store abstraction: `backend/common/vector/` with `VectorStore` base class
...
│   ├── config.py           # Environment config (ADH_*, METADATA_DB_*, VECTOR_DB_*, QDRANT_*, LANGFUSE_*)
│   ├── db/                 # MetadataDB + VectorDB connection pools (MySQL/SQLite/Doris)
│   ├── llm/                # LLM client, embedding, token estimation, Langfuse client
│   ├── vector/             # VectorStore abstraction (Qdrant / Doris HNSW / In-Memory)
...
# Full stack (MySQL + Qdrant + embedding + backend)
docker compose -f docker-compose.full.yml up -d --build

# Database sync
cd sync && python metadata_sync.py              # Sync table metadata + embeddings to vector store
...
- **Vector search uses Qdrant/Doris/In-Memory** (`VECTOR_DB_TYPE` + `QDRANT_*`/`VECTOR_DB_*` config), metadata uses MySQL/SQLite (`METADATA_DB_*` config) — separate connection pools
- **metadata_sync.py auto-syncs embeddings** to vector store (Qdrant etc.) after syncing metadata
